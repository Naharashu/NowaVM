...
-interpret - use interpretation for execution instead of JIT(by default on aarch64)

-r - prints register dump

-V - verbose mode(prints debug info)

-v - prints version

-e - executes bytecode(-e file.name if not compiling)

-Wrt - prints runtime warnings

-fno-entry0 - doesnt includes entry0.asm

-opt - turns on optimizations
